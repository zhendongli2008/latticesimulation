...
def test():
   ...
   ng = 30
   kv = []
   for idx,nc in enumerate(range(10,100,5)):
      n = 2*nc+1
      a = L/float(n)
      ...
      kv.append(vii)
   print kv
   plt.plot(nlst,abs(kv-exact),'ro-')
   #plt.plot(nlst,numpy.log10(abs(kv-exact)),'ro-')
   plt.show()
   exit()
   ...
   nt = []
   kv = []
   for idx,nc in enumerate(range(10,50,5)):
      n = 2*nc+1
      a = L/float(n)
      for ng in range(np,1000,5):
         vmpo = genEVmpoTaylor(n,a,ng,iop=1)
	 #vmpo = genEVmpo(n,a,ng)
         vii = genVij(nc,nc)
         err = numpy.log10(exact[idx]-vii)
	 print 'idx=',idx,'n=',n,'ng=',ng,'err=',err
	 if err < accuracy:
            np = ng
	    nt.append(np)
	    kv.append(vii)
	    print 'final idx=',idx,'n=',n,'ng=',ng
	    break
   plt.plot(nlst,numpy.log10(abs(kv-exact)),'ro--')
   plt.legend(loc=2)
   plt.show()
